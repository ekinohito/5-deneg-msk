...
export default function VolunteerCard({
    volunteer: { id, name, rating, picture, shortDescription },
}: Props) {
    const stars = Math.round(rating)
    return (
        <div className="bg-red-50 w-full p-4 rounded-md flex flex-row space-x-4">
            <img src={picture} className="h-32 aspect-square object-cover rounded-full" />
            <div className="flex flex-col justify-around text-lg grow">
                ...
                <Link href={`/volunteer/${id}`}><a className="self-end">Подробнее →</a></Link>
            </div>
        </div>
    )
}
